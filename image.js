...
export default class CacheableImage extends React.Component {
    constructor(props) {
        ...
        this._handleConnectivityChange = this._handleConnectivityChange.bind(
            this
        )
        ...
        RNFS.stat(filePath)
            .then(res => {
                if (res.isFile() && res.size > 0) {
                    ...
                    this.setState({
                        cacheable: true,
                        cachedImagePath: filePath
                    })
                } else {
                    throw Error(
                        'CacheableImage: Invalid file in checkImageCache()'
                    )
                }
            })
            .catch(err => {
    ...
    render() {
        ...
        return <ActivityIndicator {...this.props.activityIndicatorProps} />
    }
    ...
    renderLocal() {
        ...
        return <ResponsiveImage {...props}>{children}</ResponsiveImage>
    }
